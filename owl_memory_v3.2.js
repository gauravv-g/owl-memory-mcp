/**
 * OWL Memory MCP v3.2 — Brain-Inspired Agent Memory with NER Entity Extraction
 *
 * NEW in v3.2:
 * - LLM-based entity extraction via Xenova/bert-base-NER (local, no API key)
 * - Identifies persons, organizations, locations, misc entities from free text
 * - Replaces regex-based extraction that only caught quoted strings and capitalized names
 * - BIO tagging: B-PER, I-PER, B-ORG, I-ORG, B-LOC, I-LOC, B-MISC, I-MISC
 *
 * All from v3.1:
 ...
 *
 * All 43 tools from v3, plus vector-enhanced recall + NER entity extraction.
 */
...
const server = new Server(
  { name: "owl-memory", version: "3.2.0", description: "OWL Memory MCP v3.2 — Brain-inspired agent memory with NER entity extraction + vector embeddings. 43 tools: episodic/semantic/procedural/somatic/transactive memory, vector+BM25 hybrid search, LLM-based NER entity extraction (persons/organizations/locations), developmental stages, mutation tracking, adaptive forgetting, threat simulation, mood-congruent retrieval, predictive memory, creativity engine, memory palace, dream consolidation, spaced repetition, session checkpoints, counterfactual reasoning, and graph visualization." },
...
server.setRequestHandler(ListResourcesRequestSchema, async () => [
  ...
  { uri: "owl-memory://graph-ui", name: "Memory Graph UI", description: "Interactive force-directed graph visualization of memories, entities, and associations.", mimeType: "text/html" },
    ...
    if (name === "recall") {
      ...
      if (memoryType === "all" || memoryType === "episodic") {
        ...
        if (hasVectors && queryEmbedding && queryEmbedding.length === 384) {
          try {
            const vecRows = db.prepare("SELECT rowid, distance FROM episodic_embeddings WHERE embedding MATCH ? AND k = 50 ORDER BY distance").all(JSON.stringify(queryEmbedding));
        ...
        for (const mem of db.prepare("SELECT * FROM episodic_memories WHERE project = ? AND is_active = 1").all(projectId)) {
          ...
          if (hasVectors) {
            const embRow = db.prepare("SELECT embedding FROM episodic_embeddings WHERE rowid = ?").get(mem.id);
            if (embRow && embRow.embedding) candidateEmbeddings.set(mem.id, JSON.parse(embRow.embedding));
          }
...
async function main() {
  ...
  warmupNER();
  console.error(`OWL Memory MCP v3.2 — Brain-inspired agent memory ${hasVectors ? "+ vector embeddings" : "(no vector extension)"} ${hasNER ? "+ NER entity extraction" : "(NER loading...)"} running on stdio`);
}
