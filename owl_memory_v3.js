...
function storeEmbedding(db, rowId, emb) {
  if (!hasVectors) return;
  if (!emb || emb.length !== 384) return;
  try {
    const jsonStr = JSON.stringify(emb);
    db.exec(`INSERT OR REPLACE INTO episodic_embeddings(rowid, embedding) VALUES (${rowId}, '${jsonStr.replace(/'/g, "''")}')`);
  } catch (e) { /* ignore */ }
}
...
// --- Schema (with migration for vec_id) --------------------------------------

// Migration: ensure episodic_memories has vec_id for vector mapping
try {
  // Check if vec_id column exists
  const cols = db.prepare("PRAGMA table_info(episodic_memories)").all();
  const hasVecId = cols.some(c => c.name === "vec_id");
  if (!hasVecId) {
    db.exec("ALTER TABLE episodic_memories ADD COLUMN vec_id INTEGER");
    db.exec("UPDATE episodic_memories SET vec_id = rowid WHERE vec_id IS NULL");
  }
} catch (e) {
...
db.exec(`
    CREATE TABLE IF NOT EXISTS episodic_memories (
        vec_id INTEGER PRIMARY KEY AUTOINCREMENT, id TEXT UNIQUE NOT NULL, content TEXT NOT NULL, event_type TEXT DEFAULT 'observation',
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "remember") {
      ...
      const vecId = db.prepare("SELECT last_insert_rowid()").get()["last_insert_rowid()"];

      // Record creation mutation
      db.prepare("INSERT INTO memory_mutations (memory_id, mutation_type, new_content, new_stage, reason, created_at) VALUES (?, 'created', ?, 'raw', 'Initial storage', ?)").run(memId, content, now);
      ...
      // Store vector embedding (synchronous - await so it's available for immediate recall)
      const emb = await generateEmbedding(content);
      if (emb) {
        storeEmbedding(db, vecId, emb);
      }

      return { content: [{ type: "text", text: JSON.stringify({ memory_id: memId, event_type: eventType, emotional_valence: emotion.valence, emotional_arousal: emotion.arousal, salience: emotion.salience, strength: initialStrength, developmental_stage: "raw", next_review: nextReview, entities_extracted: entities.length, contradictions_detected: contradictionsFound, mood_tag: moodTag, sensory_type: sensoryType, vector_embedding: hasVectors && !!emb }, null, 2) }] };
    }
    ...
    if (name === "recall") {
      ...
      if (memoryType === "all" || memoryType === "episodic") {
        ...
        if (hasVectors) {
          ...
          if (queryEmb && queryEmb.length === 384) {
            try {
              ...
              const vecRows = db.prepare("SELECT rowid, distance FROM episodic_embeddings WHERE embedding MATCH ? AND k = 50 ORDER BY distance").all(queryJson);
              for (const vr of vecRows) vectorScores.set(vr.rowid, 1 - Math.min(vr.distance, 1));
            } catch (e) { /* ignore vector search errors */ }
          }
        }

        // Phase 2: BM25 + metadata
        const candidates = new Set();
        for (const mem of db.prepare("SELECT *, rowid AS rowid_val FROM episodic_memories WHERE project = ? AND is_active = 1").all(projectId)) {
          candidates.add(mem.rowid_val);
          ...
          const vecScore = vectorScores.get(mem.rowid_val) || 0;
        ...
        // Phase 3: Vector-only hits (rowid is the key)
        for (const [rowId, vecScore] of vectorScores) {
          if (!candidates.has(rowId) && vecScore > 0.3) {
            const mem = db.prepare("SELECT * FROM episodic_memories WHERE rowid = ? AND project = ? AND is_active = 1").get(rowId, projectId);
    ...
    if (name === "save_checkpoint") {
      ...
      return { content: [{ type: "text", text: JSON.stringify({ checkpoint_id: cpId, name: args.name, memories_saved: wm.length }, null, 2) }] };
    }
    ...
    if (name === "practice_skill") {
      ...
      return { content: [{ type: "text", text: JSON.stringify({ skill_id: args.skill_id, mastery_level: Math.round(nm * 100) / 100, practice_count: np, success_rate: Math.round(ns / np * 100) / 100 }, null, 2) }] };
    }
    ...
    if (name === "warn_me") {
      ...
      return { content: [{ type: "text", text: JSON.stringify({ context: args.context, threats_found: rel.length, threats: rel, message: rel.length > 0 ? `⚠️ ${rel.length} threat(s) detected.` : "No threats detected." }, null, 2) }] };
    }
    ...
    if (name === "get_mutation_history") {
      const muts = db.prepare("SELECT * FROM memory_mutations WHERE memory_id = ? ORDER BY created_at ASC").all(args.memory_id);
      return { content: [{ type: "text", text: JSON.stringify({ mutations: muts, count: muts.length }, null, 2) }] };
    }
    ...
    if (name === "health_check") {
      ...
      const threats_tracked = db.prepare("SELECT COUNT(*) as c FROM threat_patterns WHERE is_active = 1").get().c;
      const transactive_entries = db.prepare("SELECT COUNT(*) as c FROM transactive_memory").get().c;
      return { content: [{ type: "text", text: JSON.stringify({ health_score: score, status, memories: { episodic: te, semantic: ts, procedural: tp, somatic: tso, total: te + ts + tp + tso }, vector_search: hasVectors, threats_tracked, transactive_entries }, null, 2) }] };
    }
    ...
    if (name === "forget") {
      ...
      if (hasVectors && table === "episodic_memories") {
        const rowId = db.prepare("SELECT rowid FROM episodic_memories WHERE id = ?").get(args.memory_id)?.rowid;
        if (rowId) try { db.exec(`DELETE FROM episodic_embeddings WHERE rowid = ${rowId}`); } catch (e) { /* ignore */ }
      }
    ...
    if (name === "update_memory") {
      ...
      db.prepare("INSERT INTO memory_mutations (memory_id, mutation_type, new_content, new_stage, reason, created_at) VALUES (?, 'updated', ?, 'raw', 'User update', ?)").run(args.memory_id, args.new_content, now);
      if (hasVectors && table === "episodic_memories") {
        const emb = await generateEmbedding(args.new_content);
        if (emb) {
          const rowId = db.prepare("SELECT rowid FROM episodic_memories WHERE id = ?").get(args.memory_id)?.rowid;
          if (rowId) storeEmbedding(db, rowId, emb);
        }
      }
    ...
    if (name === "get_memory") {
      ...
      return { content: [{ type: "text", text: JSON.stringify({ ...m, entities, associations: assoc, mutations: muts, metacognition: metacog }, null, 2) }] };
    }
    if (name === "list_memories") {
      const pid = args.project || "default", mt = args.memory_type || "all", lim = args.limit || 50;
      const r = { episodic: [], semantic: [], procedural: [], somatic: [] };
      if (mt === "all" || mt === "episodic") {
        let q = `SELECT id, content, event_type, strength, mood_tag, developmental_stage FROM episodic_memories WHERE project = ? AND is_active = 1`;
        if (args.developmental_stage) q += ` AND developmental_stage = '${args.developmental_stage}'`;
        q += ` ORDER BY strength DESC LIMIT ?`;
        r.episodic = db.prepare(q).all(pid, lim);
      }
